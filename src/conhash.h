...
extern "C" {
    ...
    /* 
     * lookup at most C different servers where the object may belong
     * @object: the input string which indicates an object
     * @return the ips
     */
    CONHASH_API void conhash_lookup(struct conhash_s *conhash,
        const char *object, int *ipNum, struct in_addr *ips);

    /* some utility functions export*/    
    CONHASH_API void  conhash_md5_digest(const u_char *instr,
        u_char digest[16]); 
    ...
    */
    CONHASH_API void  conhash_get_vnodes(struct conhash_s *conhash,
