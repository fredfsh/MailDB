 ...
 */
#define util_rbtree_min(rbtree)\
    util_rbsubtree_min((rbtree)->root, &(rbtree)->null)

/* 
 * find the max node of tree 
 * return NULL is tree is empty
 */
#define util_rbtree_max(rbtree)\
    util_rbsubtree_max((rbtree)->root, &(rbtree)->null)
 ...
 */
util_rbtree_node_t* util_rbtree_eq_key(util_rbtree_t *rbtree, long key);

/*
 * search node with key > @key;
 * if @key > all the key values in the tree, return the node with minimum key
 * return NULL if tree is empty
 */
util_rbtree_node_t* util_rbtree_gt_key(util_rbtree_t *rbtree, long key);
 ...
 * return NULL if tree is empty
 */
util_rbtree_node_t* util_rbtree_geq_key(util_rbtree_t *rbtree, long key);
 ...
 *
 * @maxdepth contains the max length of all simple roads from root to it's
 *   leaf nodes
 */
int util_rbtree_check(const util_rbtree_t *rbtree, int *blackheight,
    int *maxdepth);
